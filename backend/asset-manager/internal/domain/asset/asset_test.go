...
func TestRichDomainModel(t *testing.T) {
	...
	t.Run("AssetPublishing", func(t *testing.T) {
		...
		video, err := asset.UpsertVideo("main", &videoFormat, *s3Object, 1920, 1080, 120.5, 5000000, "h264", 1024000000, "video/mp4", "h264", "aac", "30fps", 2, 48000, nil, nil)
		assert.NoError(t, err)
	...
	t.Run("AddVideoWithStreamInfo", func(t *testing.T) {
		...
		video, err := asset.UpsertVideo("main", &videoFormat, *s3Object, 1920, 1080, 120.5, 5000000, "h264", 1024000000, "video/mp4", "h264", "aac", "30fps", 2, 48000, streamInfo, nil)
		assert.NoError(t, err)
...
func TestDomainServices(t *testing.T) {
	...
	t.Run("AssetMetrics", func(t *testing.T) {
		...
		_, err = asset.UpsertVideo("main", &videoFormat1, *s3Object1, 1920, 1080, 120.5, 5000000, "h264", 1024000000, "video/mp4", "h264", "aac", "30fps", 2, 48000, nil, nil)
		assert.NoError(t, err)
		...
		_, err = asset.UpsertVideo("trailer", &videoFormat2, *s3Object2, 1280, 720, 60.0, 2000000, "h264", 512000000, "application/x-mpegURL", "h264", "aac", "30fps", 2, 48000, nil, nil)
		assert.NoError(t, err)
	...
	t.Run("StorageUsage", func(t *testing.T) {
		...
		s3Object1, _ := valueobjects.NewS3Object("test-bucket", "videos/main.mp4", "https://test-bucket.s3.amazonaws.com/videos/main.mp4")
		videoFormat1 := valueobjects.VideoFormat(constants.VideoStreamingFormatRaw)
		video1, err := asset.UpsertVideo("main", &videoFormat1, *s3Object1, 1920, 1080, 120.5, 5000000, "h264", 1024000000, "video/mp4", "h264", "aac", "30fps", 2, 48000, nil, nil)
		assert.NoError(t, err)
		video1.UpdateSize(1024 * 1024 * 100)

		s3Object2, _ := valueobjects.NewS3Object("test-bucket", "videos/trailer.m3u8", "https://test-bucket.s3.amazonaws.com/videos/trailer.m3u8")
		videoFormat2 := valueobjects.VideoFormat(constants.VideoStreamingFormatHLS)
		video2, err := asset.UpsertVideo("trailer", &videoFormat2, *s3Object2, 1280, 720, 60.0, 2000000, "h264", 512000000, "application/x-mpegURL", "h264", "aac", "30fps", 2, 48000, nil, nil)
		assert.NoError(t, err)
