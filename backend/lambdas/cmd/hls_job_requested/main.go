...
func NewHLSJobRequestedEvent(assetID, videoID, input string) *HLSJobRequestedEvent {
	return &HLSJobRequestedEvent{
		AssetID: assetID,
		VideoID: videoID,
		Input:   input,
	}
...
func (e *HLSJobRequestedEvent) ToCloudEvent() *pkgevents.Event {
	event := pkgevents.NewEvent("hls.job.requested", e)
...
func handleHTTPRequest(ctx context.Context, request awsevents.APIGatewayProxyRequest) (awsevents.APIGatewayProxyResponse, error) {
	...
	log := logger.WithService("hls-job-requested-lambda")
	...
	event := NewHLSJobRequestedEvent(input.AssetID, input.VideoID, input.Input)

	if err := producer.SendEvent(ctx, "hls.job.requested", event.ToCloudEvent()); err != nil {
	...
	log.Info("HLS job requested event sent successfully", "asset_id", input.AssetID, "video_id", input.VideoID)

	return awsevents.APIGatewayProxyResponse{
		StatusCode: 200,
		Headers:    corsHeaders(),
		Body:       fmt.Sprintf(`{"message": "HLS job requested successfully", "assetId": "%s", "videoId": "%s"}`, input.AssetID, input.VideoID),
	}, nil
}
