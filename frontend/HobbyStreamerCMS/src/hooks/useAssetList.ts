  ...
  const handleUpdateAsset = async (field: string, value: any) => {
    ...
    try {
      ...
      const updateData: any = {
        ...
        genre: selectedAsset.genre,
        genres: selectedAsset.genres,
      ...
      if (field === 'title') {
        ...
        if (value === '') {
          ...
          clearFields.push('genre');
        } else {
          updateData.genre = value;
        }
      } else if (field === 'additionalGenres') {
        if (value.length === 0) {
          updateData.genres = null;
          clearFields.push('genres');
        } else {
        ...
        if (value.length === 0) {
          ...
          clearFields.push('tags');
        } else {
          updateData.tags = value;
        }
      }

      const updatedAsset = await assetService.updateAsset(selectedAsset.id, updateData, clearFields);
